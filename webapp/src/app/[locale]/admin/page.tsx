  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          {RANGES.map((r) => (
            <Link
              key={r}
              href={{ pathname: "/admin", query: r === 30 ? {} : { days: String(r) } }}
              className={cn(
                buttonVariants({ variant: r === days ? "primary" : "outline", size: "sm" }),
              )}
            >
              {r} kun
            </Link>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-5">
        ...
        <StatCard label="Ochiq vakansiyalar" value={totals.jobsOpen} hint={`Jami: ${groupNumber(totals.jobsTotal)}`} />
        <StatCard label="Arizalar" value={totals.applications} />
        <StatCard label="Kompaniyalar" value={totals.companies} hint={`Tasdiqlangan: ${groupNumber(totals.companiesVerified)}`} />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Section title="Top kategoriyalar (ochiq vakansiyalar)">
          <BarList items={stats.topCategories.map((c) => ({ label: c.name, value: c.n }))} />
        </Section>
        <Section title="Top shaharlar (ochiq vakansiyalar)">
          <BarList items={stats.topCities.map((c) => ({ label: c.city, value: c.n }))} />
        </Section>
      </div>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard label="Jami daromad (so'm)" value={finance.revenueTotal} />
        <StatCard label="Hamyon majburiyati (so'm)" value={finance.walletLiability} />
        <StatCard
