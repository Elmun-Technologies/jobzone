...
| `WEBAPP_URL` | telegram-channel-post (job link in channel posts) | your production web domain, e.g. `https://yolla.uz` — *optional, defaults to that* |
...
supabase functions deploy notify-dispatch push-dispatch saved-search-alerts \
  send-sms-hook telegram-webhook telegram-channel-post
```
...
6. **Telegram channel auto-post:** map a category+region to a test channel in
